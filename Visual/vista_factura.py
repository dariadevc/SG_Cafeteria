...
class PDF(FPDF):
    ...
    def crear_factura(
        self, nro_factura, fecha, lista_pedido, nro_mesa, metodo_pago, empleado, dni
    ):
        ...
        pdf.image("C://Users//camus//Desktop//SG_Cafeteria//Visual//imagenes//coffe.png", x=0, y=25, w=30,h=25)
        # pdf.image(
        ...
        pdf.set_font("Arial", "", 12)
        pdf.cell(
            200, 5, txt=f"Nro. de factura : {self.__nro_factura}", ln=True, align="L"
        )
        pdf.cell(200, 5, txt=f"Fecha : {self.__fecha}", ln=True, align="L")
        pdf.cell(200, 5, txt=f"Nro. de mesa : {self.__nro_mesa}", ln=True, align="L")
        pdf.cell(
            200, 5, txt="Codigo  Cant.  Precio Uni.  Precio tot.", ln=True, align="L"
        )

        self.__total = 0
        self.__descuento = 0
        for i in range(0, canti_filas):
            self.__cod_producto, self.__cantidad, self.__precio_unitario = (
                self.__lista_pedido[i]
            )
            ...
            pdf.cell(
                0,
                10,
                "{:<6}    {:<6}    {:<6.2f}         {:>6.2f}".format(
                    self.__cod_producto,
                    self.__cantidad,
                    self.__precio_unitario,
                    self.__parcial_total,
                ),
                0,
                1,
            )
        ...
        pdf.cell(
            200,
            5,
            txt=f"Subtotal $ {self.__total:.2f}          Descuento $ {self.__descuento:.2f} ",
            ln=True,
            align="L",
        )
        pdf.cell(200, 5, txt=f"Total $ {self.__total:.2f}", ln=True, align="L")
        pdf.cell(
            200, 5, txt=f"Metodo de pago : {self.__metodo_pago}", ln=True, align="L"
        )
        pdf.set_font("Arial", "B", 12)
        pdf.cell(
            200,
